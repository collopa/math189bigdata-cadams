...
def find_cost(X, y, W , reg):
	...
	tot_err =  float(error.T @ error) #(error * error).sum()

	cost = (tot_err + reg * np.abs(W).sum()) / len(y)
...
def find_grad(X, y, W, reg=0.0):
	...
	grad = np.matmul(Xt, np.matmul(X, W) - y) / X.shape[0]
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
	...
	for index in range(num_reg):
		...
		# 		as they are.
		# """*** YOUR CODE HERE ***"""
		W[:, index] = grad_lasso(X, y, \
			batch_size = 2**10, print_freq = 1000, reg = reg)[0].flatten()
		#that's it????
		# """*** END YOUR CODE HERE ***"""
...
if __name__ == '__main__':
	...
	# """*** YOUR CODE HERE ***"""
	# five_inds = np.argsort(W[:,0])[:5]
	# cols = df.columns
	# top_features = np.array(cols)

	# for ind in five_inds:
	# 	top_features += [cols[ind + 1]]

	##relied on solutions here
