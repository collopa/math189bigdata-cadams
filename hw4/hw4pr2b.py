...
def NLL(X, y, W, reg=0.0):
	...
	mu = np.matmul(X,W)
	prob = np.exp(mu)/(np.exp(mu).sum(axis=1).reshape(-1, 1))
	actual = y * np.log(prob)

	NLL = - actual.sum(axis=1).sum() + reg*np.diag(np.matmul(W.T, W)).sum()
...
def grad_softmax(X, y, W, reg=0.0):
	...
	grad = np.matmul(Xt, (prob - y)) + reg*W
	return grad
...
def grad_descent(X, y, reg=0.0, lr=1e-5, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		grad = grad_softmax(X, y, W, reg=reg)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	best_reg = np.argmax(accu_list)
	reg_opt = lambda_list[best_reg]
