...
def grad_logreg(X, y, W, reg=0.0):
	...
	Xt = np.transpose(X)
	inside = np.matmul(X,W)

	grad = np.matmul(Xt, sigmoid(inside) - y) + reg * W
...
def NLL(X, y, W, reg=0.0):
	...
	nll = np.multiply(y, np.log(mu)) + np.multiply((1. - y), np.log(1. - mu))
	nll = - nll.sum() + reg * np.linalg.norm(W)**2
...
def newton_step(X, y, W, reg=0.0):
	...
	diag = np.diag(np.squeeze(np.asarray(np.multiply(mu, 1. - mu)))) ## had to copy this from solutions

	Hess = np.matmul(np.transpose(X), np.matmul(diag, X)) + reg*np.eye(X.shape[1])
...
def get_description(X, y, W):
	...
	for i in range(X.shape[0]):
		actual, pred = y.item(i), ypred.item(i)
		if actual == pred:
			acc_num += 1
		if actual == 1:
			total_r += 1
	...
	accuracy = 1. * acc_num/X.shape[0]
	precision = 1. * pred_num/total_p
	recall = 1. * recall_num/total_r
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"""
	# reg_list = np.linspace(0, 5, 10)
	reg_list = np.linspace(3,4, 10)
	a_list = []
	p_list = []
	r_list = []
	f1_list = []
	...
	for lmbda in reg_list:
		W_test, _ = grad_descent(X_train, y_train, reg = lmbda, \
			lr = 2e-4, print_freq = 100)
		a, p, r, f1 = get_description(X_test, y_test, W_test)
		a_list += [a]
		p_list += [p]
		r_list += [r]
		f1_list += [f1]
