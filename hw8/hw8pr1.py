...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			data = X[i, :]
			diff = data - clusters
			temp_label = np.argsort(np.linalg.norm(diff, axis = 1)).item(0)
			label[i] = temp_label
		#step 3
		for cl_num in range(k):
			index = np.where(label == cl_num)[0] #had to look up this function
			if len(index) >= 1:
				clusters[cl_num, :] = X[index].mean(axis = 0)
		# "*** END YOUR CODE HERE ***"
...
def k_means_cost(X, clusters, label):
	...
	# "*** YOUR CODE HERE ***"
	Xcl = clusters[label.flatten()]
	cost = (np.linalg.norm(X - Xcl, axis = 1) ** 2).sum()
	# "*** END YOUR CODE HERE ***"
	return cost
...
if __name__ == '__main__':
	...
	# "*** YOUR CODE HERE ***"
	plt.xkcd()
	plt.plot(range(1,21), cost_k_list, 'o')

	plt.plot(opt_k, cost_k_list[opt_k - 1], 'r^', label = "Optimal Cluster")
	...
	plt.title('Cost vs Number of Clusters', size = 24)
	...
	data_plot = plt.plot(X[:,0], X[:, 1], 'o' , \
					color = 'xkcd:gray', alpha = 0.65, label = "Data")
	center_plot = plt.plot(X_cluster[:, 0], X_cluster[:, 1], 'rx', label = 'Cluster')
	plt.legend()
	plt.tight_layout()
	...
	plt.title('Visualization with {} clusters'.format(opt_k), size = 20)
