...
def wrapper(path_in, split_col, savePath, col_score_split=['user','label'],
    ...
    for idx, model in enumerate(modelList):
        ...
        for d_set in data:
            ...
            # acc_feats += [f for f in all_feats if 'curv' in f]
            # acc_feats += [f for f in all_feats if 'tor' in f]
            acc_feats += [f for f in all_feats if 'Acc' in f]
            acc_feats = sorted(acc_feats)
            clf.load_data(d_set, acc_feats)

            # for custom data
            # clf.load_data(d_set)

            # for custom without curv/tors
            # hc_feats = sorted([i for i in all_feats if 'Acc' in i])
            # clf.load_data(d_set, hc_feats)
...
#     scores = clf.crossval(split_col='user')
    ...
    #                                      'splits':['user', 'label']}
                   # }
    # ms_normal = '/Users/collopa/Desktop/nonlinear/project/data/custom/MotionSense_FeatMat_256.csv'
    # ms_normal = '/Users/collopa/Desktop/nonlinear/project/data/custom/MotionSense_FeatMat_256.csv'
    ms_normal = '/Users/collopa/Desktop/nonlinear/project/data/ms_raw_and_inv.csv'
    ma_normal = '/Users/collopa/Desktop/nonlinear/project/data/ma_raw_and_inv.csv'

    # cross_dataset = {'cross_dataset_normal': {'data': [ms_normal, ma_normal],'cv_col':'user',
    #                                           'save_path' : '//Users/collopa/Desktop/nonlinear/project/data/results/custom/ms_v_ma_boring/',
    #                                           'splits':['user','label','dataset']}}

    cross_dataset = {'cross_dataset_normal': {'data': [ms_normal, ma_normal],'cv_col':'dataset',
                                              'save_path' : '/Users/collopa/Desktop/nonlinear/project/data/results/raw_and_cust/',
                                              'splits':['user','label','dataset']}}
