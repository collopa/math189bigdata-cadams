...
if __name__ == '__main__':
	...
	img = PIL.Image.open(urllib.request.urlopen('http://i.imgur.com/X017qGH.jpg'))
	img = np.array(img)
	...
	img_copy = img_flat.copy()
	np.random.shuffle(img_copy)
	shuffle_img = img_copy
	...
	U_s, S_s, V_s = np.linalg.svd(shuffle_img, full_matrices=True)
	...
	# "*** YOUR CODE HERE ***"
	orig_S_plot = plt.plot(S[:k], c='xkcd:red', label = 'original')
	shuf_S_plot = plt.plot(S_s[:k], c='xkcd:blue', label = 'shuffled')
	...
	for index in range(len(rank_list)):
		...
		"*** YOUR CODE HERE ***"
		recon = np.matmul(U[:, :k] , np.matmul(np.diag(S)[:k, :k], V[:k, :] ) )
