...
def linreg(X, y, reg=0.0):
	...
	# "*** YOUR CODE HERE ***"
	size = X.shape
	id_mat = np.eye(size[1])
	#want the constant term to go away
	id_mat[0,0] = 0

	#want to find matrix to solve
	Xtrans = np.transpose(X)
	regM = reg * id_mat
	W_opt = np.linalg.solve(np.matmul(Xtrans, X) \
							+ regM, np.matmul(Xtrans, y) )
...
def find_RMSE(W, X, y):
	...
	RMSE = np.linalg.norm(diff)**2/size
	RMSE = np.sqrt(RMSE)
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	# "*** YOUR CODE HERE ***"
	reg_list = np.random.uniform(0, 150, size = 300)
	reg_list = np.sort(reg_list)

	W_list = [linreg(X_train, y_train, reg = reg_l) for reg_l in reg_list]
	
	RMSE_list = []
	for i in range(len(W_list)):
		RMSE_list += [find_RMSE(W_list[i], X_val, y_val)]
	...
	plt.xkcd()
	plt.setp(RMSE_vs_lambda_plot, color='xkcd:blue', lw = 3)
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	norm_list = []
	for i in range(len(W_list)):
		norm_list += [np.linalg.norm(W_list[i])**2]
	...
	plt.setp(norm_vs_lambda_plot, color='xkcd:brick', lw = 3)
...
def linreg_no_bias(X, y, reg=0.0):
	...
	sz = len(y)
	mid_mat = np.eye(sz) - np.ones(sz)/sz

	next_mat = np.matmul(X_trans, mid_mat)

	W_opt = np.linalg.solve( np.matmul(next_mat, X) + reg*np.eye(next_mat.shape[0]), \
								np.matmul(next_mat, y) )

	b_opt = 1/sz * np.dot(np.ones(sz), y - np.matmul(X, W_opt))
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		# "*** YOUR CODE HERE ***"
		train_rmse = np.linalg.norm( np.matmul(X_train, W).reshape((-1, 1)) + b - y_train)
		train_rmse = train_rmse**0.5/m_train
		obj_train += [train_rmse]

		vrmse = np.linalg.norm( np.matmul(X_val, W).reshape((-1, 1)) + b - y_val)
		vrmse = vrmse**0.5/m_val
		obj_val += [vrmse]


		X_train_trans = np.transpose(X_train)
		A_train = np.matmul(X_train_trans, X_train) + reg * np.eye(n)

		W_grad = (np.matmul(A_train, W) + np.matmul(X_train_trans, b - y_train))/m_train

		b_grad = np.dot(np.ones(len(y_train)), np.matmul(A_train, W) - m_train * b + y_train)/m_train
	...
	plt.setp(train_rmse_plot, color='red', lw = 3)
	val_rmse_plot, = plt.plot(range(iter_num), obj_val)
	plt.setp(val_rmse_plot, color='green', lw = 3)
